...
//                (select responsibility_param.fx_category_counterparty from responsibility_param  where fx_responsibility = '.$user->respon_id.')';
    ...
    }elseif(!empty($_REQUEST['lineactive']) && $_REQUEST['lineactive'] == -1){
...
//    die();
    ...
    $sql_count .= ' and `llx_societe`.`rowid` in (' . implode(',', $filterarray) . ')';
}else{
    if(isset($_REQUEST['lineactive'])&& !empty($_REQUEST['lineactive']) && $_REQUEST['lineactive'] != -1) {
...
function ClearFilterMessage(){
        ...
        die('<span style="font-size: 20px">Не знайдено жодного контрагента. Натисніть кнопку <button style="height: 25px" onclick="clearfilter();">Зняти фільтр</button></span>');
}
