...
include $_SERVER['DOCUMENT_ROOT'].'/dolibarr/htdocs/theme/'.$conf->theme.'/responsibility/purchase/action.html';
//llxFooter();
llxPopupMenu();
exit();
...
function ShowActionTable(){
    ...
    $sql = "select `llx_actioncomm`.id as rowid, `llx_actioncomm`.`datep`, `llx_societe_action`.dtChange as `datec`, `llx_user`.lastname,
        ...
        where fk_soc = ".(empty($_REQUEST['socid'])?0:$_REQUEST['socid'])." and `llx_actioncomm`.`active` = 1 and  `llx_societe_action`.id_usr = ".$user->id."
        union
        ...
        and `llx_societe_action`.id_usr = ".$user->id."
        order by `datep` desc";
