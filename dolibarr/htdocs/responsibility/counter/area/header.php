...
$sql = 'select regions.rowid, regions.state_id, trim(states.name) as states_name, trim(regions.name) as regions_name from states, regions, '.MAIN_DB_PREFIX.'user_regions ur
    where ur.fk_user='.$id_usr.' and ur.active = 1 and ur.fk_id=regions.rowid and regions.state_id=states.rowid order by regions_name asc, states_name asc';
//die($sql);
$res = $db->query($sql);
...
if($db->num_rows($res) > 0) {
    ...
    $Classifycation .= '<tr><td>Кільк.контрагентів</td><td class="autoinsert" id="iCount"></td></tr>';
