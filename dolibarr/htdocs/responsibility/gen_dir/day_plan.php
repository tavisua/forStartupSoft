...
function getOverdueUserID(){
    ...
    $out = array();
    while($obj = $db->fetch_object($res)){
        if(!in_array($obj->fk_user_action, $out))
            $out[]=$obj->fk_user_action;
        if(!empty($obj->fk_element)&&!in_array($obj->fk_element, $out))
            $out[]=$obj->fk_user_action;
    }
    return $out;
}
...
//    var_dump($subdiv_id, $class, $code);
//    die();
    if(!isset($_SESSION['overdueUserID'])) {
        $_SESSION['overdueUserID'] = getOverdueUserID();
    }
//    echo '<pre>';
//    var_dump(implode(',', $_SESSION['overdueUserID']));
//    echo '</pre>';
//    die();
