...
//    $sql.=" inner join (select `llx_user`.rowid, `responsibility`.`alias` from `llx_user` inner join `responsibility` on `responsibility`.`rowid` = `llx_user`.`respon_id` where `llx_user`.`subdiv_id` in (".implode(',',$user->getmentors($user->id)).") and `llx_user`.`active` = 1) sub_user on sub_user.rowid = case when llx_actioncomm_resources.fk_element is null then llx_actioncomm.`fk_user_author` else llx_actioncomm_resources.fk_element end";
//else
...
//        console.log($("#reference_body").find("button"));
        $.each($("#reference_body").find("button"), function(key, btn){
             var onclick = $("#"+btn.id).attr("onclick");
             var start = strpos(onclick, "(");
             var end = strpos(onclick, ")")+1;
             var param = onclick.substr(start, end-start); 
             $("#"+btn.id).attr("onclick", "ShowAllTask"+param);
//             console.log($("#"+btn.id), param);
        })           
    })
    </script>';
}
...
//        $out.= getTotalSubdivAction('','Всього задач',$subdiv);
//        $out .= getTotalSubdivAction('', 'Всього "Глобальні"', $subdiv, 'AC_GLOBAL');
//        $out .= getTotalSubdivAction('', 'Всього "Поточні"', $subdiv, 'AC_CURRENT');
...
function getTotalSubdivAction($class,$title,$subdiv_id=0,$code=''){
    ...
    $out = '<tr class="'.$class.'">';
    $start = time();
    if(!is_array($subdiv_id))
        $sql="select name from subdivision where rowid = ".$subdiv_id;
    else
        $sql="select name from subdivision where rowid IN (".implode(',',$subdiv_id).")";

    $res = $db->query($sql);
    if (!$res)
        dol_print_error($db);
    $obj=$db->fetch_object($res);
    $out.='<td colspan="2">'.$title.(!empty($subdiv_id)&&!is_array($subdiv_id)?(' '.trim($obj->name).'.'):'').'</td>';
    if(!empty($subdiv_id))
        $out.='<td></td>';
    else
...
//Всього завдань та виконані
    ...
    for($i=0; $i<=1; $i++) {
        ...
        if(!empty($subdiv_id)&&!is_array($subdiv_id))
            $sql.=" and llx_user.subdiv_id = " . $subdiv_id;
        elseif (is_array($subdiv_id)&&count($subdiv_id)>0)
...
//        die();
        ...
        $today = new DateTime();
        foreach ($actions as $action){
            //Підрахунок загальної кількості дій
            if(!empty($subdiv_id)&&!is_array($subdiv_id)&&$action['subdiv_id']==$subdiv_id || is_array($subdiv_id)&&count($subdiv_id)>0&&in_array($action['subdiv_id'], $subdiv_id)) {
                $TotalCount++;
//                var_dump(date('Y-m-d', strtotime($action['datep'])),$action['datep']);
//                die();
                if(empty($total[date('Y-m-d', strtotime($action['datep']))]))
                    $total[date('Y-m-d', strtotime($action['datep']))] = 1;
                else
                    $total[date('Y-m-d', strtotime($action['datep']))]++;
                
                if(new DateTime($action['datep'])<=$today&&$action['percent'] == 100) {//Підрахунок фактично виконаних дій
                        if(empty($fact[date('Y-m-d', strtotime($action['datep']))]))
                            $fact[date('Y-m-d', strtotime($action['datep']))]=array();
                        if (empty($code) && (in_array($action['code'], array('AC_GLOBAL', 'AC_CURRENT')) || $action['callstatus'] == 5)) {
                            if (empty($fact[date("Y-m-d", strtotime($action['datep']))]))
                                $fact[date("Y-m-d", strtotime($action['datep']))] = array();
                            if (!in_array($action['rowid'], $fact[date('Y-m-d', strtotime($action['datep']))]))
                                $fact[date("Y-m-d", strtotime($action['datep']))][] = $action['rowid'];
                        } elseif (in_array($code, array('AC_GLOBAL', 'AC_CURRENT')) || $action['callstatus'] == 5) {
                            if (!in_array($action['rowid'], $fact[date('Y-m-d', strtotime($action['datep']))]))
                                $fact[date("Y-m-d", strtotime($action['datep']))][] = $action['rowid'];
                        }

                }elseif(new DateTime($action['datep'])>=$today&&$action['percent'] != 100) {//Майбутні
                    if (empty($future[date('Y-m-d', strtotime($action['datep']))]))
                        $future[date('Y-m-d', strtotime($action['datep']))] = array();
                    if (!in_array($action['rowid'], $future[date('Y-m-d', strtotime($action['datep']))]))
                        $future[date('Y-m-d', strtotime($action['datep']))][] = $action['rowid'];

                }elseif(new DateTime($action['datep'])<$today&&$action['percent'] != 100)//Прострочені
                    $OutStanding++;
            }
        }
        ...
        foreach (array_keys($fact) as $key){
            if(!in_array($key, array('week', 'month'))) {
                $datep = new DateTime($key);
                if ($datep <= $today) {
                    if ($today->diff($datep)->format('%d') <= 7)
                        $fact['week'] += count($fact[$key]);

                    $fact['month'] += count($fact[$key]);
                }
            }
        }
        $future['week'] = 0;
        foreach (array_keys($future) as $key){
            if(!in_array($key, array('week', 'month'))) {
                ...
                if ($datep <= $today) {
                    if ($today->diff($datep)->format('%d') <= 7)
                        $future['week'] += count($future[$key]);
    
                    $future['month'] += count($future[$key]);
                }
            }
        }    
//        $total[$period] = $TotalCount;
        //Фактично виконаних
//        $sql = "select  count(distinct llx_actioncomm.id) iCount from llx_actioncomm
//            left join `llx_actioncomm_resources` on `fk_actioncomm` = llx_actioncomm.id
//            left join llx_user on llx_user.rowid = case when `llx_actioncomm_resources`.`fk_element` is null then `fk_user_author` else `llx_actioncomm_resources`.`fk_element` end";
...
$percent_block = '';
    ...
    for($i=8; $i>=0; $i--){
        if($i < 8) {
            $percent = 0;
            if($i<7) {
                $count = count($fact[date("Y-m-d", (time() - 3600 * 24 * $i))]);
                $total_count = count($total[ date("Y-m-d", (time() - 3600 * 24 * $i))]);
            }else{
                $count = $fact['week'];
                $total_count = $total['week'];
            }
...
//        die('fact'.date("Y-m-d", (time()-3600*24*$i)));
        if(isset($fact[date("Y-m-d", (time()-3600*24*$i))]))
            $fact_block.='<td class="middle_size" style="text-align: center">'.count($fact[date("Y-m-d", (time()-3600*24*$i))]).'</td>';
        else
            $fact_block.='<td class="middle_size" style="text-align: center"></td>';
    }
...
//    $obj = $db->fetch_object($res);
    if (empty($OutStanding))
        ...
        $outstanding = '<td  style="text-align: center">' .$OutStanding . '</td>';
    //майбутнє
//    $sql = "select date(datep) datep, count(distinct llx_actioncomm.id) iCount from llx_actioncomm
//        left join `llx_actioncomm_resources` on `fk_actioncomm` = llx_actioncomm.id
//        left join llx_user on llx_user.rowid = case when `llx_actioncomm_resources`.`fk_element` is null then `fk_user_author` else `llx_actioncomm_resources`.`fk_element` end";
...
//    die();
    ...
    for($i=0;$i<=8;$i++){
        ...
        if($i<=6) {
            if(isset($future[$date]))
                $future_block .= '<td  style="text-align: center">'.count($future[$date]).'</td>';
            else
                $future_block .= '<td></td>';
        }else {
//            $sql = "select count(id) iCount  from llx_actioncomm
...
//            $obj = $db->fetch_object($res);
            ...
            if (empty($obj->iCount))
                ...
                $future_block .= '<td  style="text-align: center">' . $Count . '</td>';
        }
    }
    $out.=$percent_block.$fact_block.$outstanding.$future_block;
//    echo '<pre>';
