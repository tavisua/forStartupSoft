...
<table class="nobordernopadding<?php if ($force_install_noedit) print ' hidewhennoedit'; ?>">
	...
	<tr class="hidesqlite">
		...
		<td valign="top" class="label"><input type="text"
			...
			value="<?php print (! empty($dolibarr_main_db_host))?$dolibarr_main_db_host:(empty($force_install_dbserver)?'127.0.0.1':$force_install_dbserver); ?>">
	...
	<tr class="hidesqlite hideroot">
		...
		<td class="label" valign="top"><input type="password" autocomplete="off"
			...
			if (! empty($_SERVER["SERVER_NAME"]) && ! in_array($_SERVER["SERVER_NAME"], array('127.0.0.1', '127.0.0.1'))) $autofill='';	// Do not autofill password for remote access
			print dol_escape_htmltag($autofill);
			?>"></td>
