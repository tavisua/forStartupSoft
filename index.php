...
$Link = '/dolibarr/htdocs/index.php';
...
if (is_dir($aliasDir))
{
	...
	foreach($listoffiles as $file) 
	{
	    ...
	    {		
			...
			if (preg_match('/dolibarr/i',$file)) $aliasContents .= $file.'</a></li></ul></td><td>http://127.0.0.1'.($apachePort != 80?':'.$apachePort:'').'/'.$file.'/</td><td>http://<i>ipofyourserver</i>'.($apachePort != 80?':'.$apachePort:'').'/'.$file.'/</td></tr>';
			elseif (preg_match('/phpmyadmin/i',$file)) $aliasContents .= $file.'</a></li></ul></td><td>http://127.0.0.1'.($apachePort != 80?':'.$apachePort:'').'/'.$file.'/</td><td>'.$langues[$langue]['NotAvailable'].'</td></tr>';
...
foreach($listoffiles as $file)
{
	...
	{		
		...
		$projectContents .= '</ul></td><td>http://127.0.0.1'.($apachePort != 80?':'.$apachePort:'').'/'.$file.'/)'.'</td><td>'.$langues[$langue]['NotAvailable'].'</td></tr>';
	}
}
